...
### Memory Architecture (was `mona-enhanced-memory`)

Memory layer for Mona — JSON-based user/project memory at `~/.hermes/memories/`:
...
**Trigger conditions:**
- User says "inget ini", "catat", "jangan lupa" → write to memory
- User corrects Mona's behavior → log to `correction_log.json`
- User shares preferences/habits → update `user_preferences.json`
- Session ends with unresolved context → write `session_summaries/<date>.md`

**Priority order when loading:** user_preferences > important_facts > project_context > learned_patterns > correction_log. Communication style (Indonesian casual, "sayang", concise) lives at the top.

### Related skills (kept standalone — distinct domains)
- `mona-emotional-intelligence` — Persona layer: empathy-first response, holding boundaries, recognizing boundary-testing vs genuine requests, de-escalation. NOT about bot plumbing — about how Mona TALKS to the user.
- `mona-knowledge-vault` — Obsidian-style Git-backed vault (separate from JSON memory). Different mechanism, different tools (sync-agents-md.sh, vault templates). Use for long-form audit trails and multi-device sync.
- `mona-provider-health` — Provider stack monitoring (MiMo/9Router/OpenRouter). Substantial scripts (`provider-health-check.sh`, `kimchi_bulk_add.py`) and references — too broad to fold here.

## Where things moved

- `mona-enhanced-monitoring` (whale tracking with multi-provider fallback) → absorbed into `hermes-crypto-agent` under `references/smart-money-tracker.md` + `references/multi-provider-fallback.md`. Reason: the architecture is crypto-specific (Alchemy → Zerion → Helius chain), not bot-specific.

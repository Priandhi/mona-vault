...
The following skills were archived after their unique content was folded into this umbrella's references and scripts. When any of these topics come up, this umbrella is the load target — the absorbed references below remain the authoritative detail.

### Trading Engines & Signal Generation
- **`crypto-futures-engine`** → absorbed. Full Binance/Bybit/OKX perpetual futures engine lives in `references/futures-engine.md` + `references/futures-engine-fixes.md`. All 51 session-specific learning files migrated into this umbrella's references/. Safety rails: `references/autonomous-safety-rails.md`.
- **`crypto-signal-scanner`** → absorbed. Dual-mode momentum + reversal scanning pattern in `references/dual-mode-scanner.md` + `references/dual-mode-trading.md`. SMC engine integration in `references/dozero-smc-integration.md`.
- **`binance-futures-trading`** → absorbed. Binance algo orders endpoint quirk (STOP_MARKET/TAKE_PROFIT_MARKET moved to `/fapi/v1/algoOrder`) documented in `references/binance-algo-order-api.md`. Auto SL/TP manager in `references/auto-sltp-manager.md`. Leverage bracket API in `references/leverage-bracket-api.md`.

### Sniper Bots (Solana + Charon)
- **`charon-sniper-bot`** → absorbed. Charon API field reference in `references/charon-api.md` + `references/charon-api-and-sniper-bot.md`. Bot templates preserved at `.archive/charon-sniper-bot/templates/` (solana-bot-watcher.sh, solana-bot-autoswap.js, solana-keypair-convert.js). Advanced token filter in `references/advanced-token-filter.md`.
- **`solana-sniper-bot`** → absorbed. Generic Solana sniper pattern with Jupiter DEX in `references/solana-trading-bot.md` + `references/solana-wallet-import.md`. Jupiter API notes in `references/jupiter-api.md`.

### LP Agent
- **`meridian-dlmm-agent`** → absorbed. Meridian Meteora DLMM setup in `references/meridian-setup.md`. Position lifecycle + close rules in `references/meridian-position-lifecycle.md`. Telegram conflict resolution in `references/meridian-telegram-conflict.md`. Small-wallet preset in `references/very-small-wallet-preset.md`. Threshold tweaking guide in `references/threshold-tweaking-guide.md`. Scripts preserved at `.archive/meridian-dlmm-agent/scripts/` (screening_tracker.py, monitor.sh).

### Alpha Discovery & Intel
- **`alpha-hunter-new-token-discovery`** → absorbed. New-token scanning workflow in `references/alpha-hunting-workflow.md`. Implementation notes in `.archive/alpha-hunter-new-token-discovery/references/` (sniper-monitor-impl.md, browser-scraping-fallback.md, api-examples.md).
- **`alpha-intel-system`** → absorbed. KOL + Twitter scanner pattern, smart money pitfalls in `.archive/alpha-intel-system/references/` (smart-money-pitfalls.md, telegram-groups.md, web-search-alpha-hunter.md).
- **`crypto-smart-money`** → absorbed. Real-time tracker pattern in `references/smart-money-tracker.md`. Multi-provider fallback in `references/multi-provider-fallback.md`. GoPlus security API in `references/goplus-security-api.md`. Zerion API in `references/zerion-api.md`. Deployer detection via Alchemy in `references/deployer-detection-alchemy.md`.

### Alerting
- **`crypto-alert-bot`** → absorbed. Alert architecture in `references/alert-system-architecture.md`. Format templates in `references/alpha-alert-template-design.md` + `references/macd-alert-format.md`. Telegram topic setup in `references/telegram-topic-creation.md` + `references/telegram-topic-api.md`.

### Data Acquisition
- **`crypto-data-scrapers`** → absorbed. Endpoint catalog in `references/data-sources.md`. DeFiLlama + DexScreener curl recipes preserved. Cron pattern every 10m for pairs / 6h for TVL.

### Airdrop Mechanics
- **`galxe-reverse-engineering`** → absorbed. Full Galxe GraphQL schema in `references/galxe-api-schema.md`. Automation pipeline in `references/galxe-automation.md`. WASM captcha bypass in `references/galxe-wasm-captcha-bypass.md`. Auto-pipeline from Telegram channels in `references/airdrop-executor.md`. Zero-capital farming pattern in `references/airdrop-farming-zero-modal.md`.

### Mining Infrastructure (distinct workflows — KEEP standalone)
- `gpu-cloud-mining` — Modal/RunPod/Vast.ai GPU mining setup is its own infrastructure domain. Kept standalone.
- `vps-mining-setup` — VPS CPU mining (RandomX/Juno) is its own infrastructure domain. Kept standalone.

### Web3 Game Bypass
- **`web3-game-bypass-recon`** → absorbed. Methodology (DNS→origin, JS bundle mining, JWT brute force, token contract analysis) inline in this umbrella. Case study at `.archive/web3-game-bypass-recon/references/owntown-case-study.md`.

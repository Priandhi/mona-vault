...
description: "Honest technical review of proposals from other agents (Claude, GPT, etc.), blog posts, docs, or external sources. Core discipline: only flag ACTUAL issues with verified evidence; never fabricate bugs to look thorough; own mistakes immediately when wrong. Trigger when user asks to review a technical proposal, says 'koreksi dulu', 'review', 'kasih feedback', or whenever the agent is about to claim something is wrong/broken."
when_to_use:
  - "User asks Mona to review another agent's suggestion (Claude, GPT, Gemini, etc.)"
  - "Reviewing a config, script, plan, or proposal from a blog post, doc, or external source"
  - "User says 'koreksi', 'review dulu', 'kasih feedback', 'ada yang salah gak?'"
  - "Before claiming 'X is wrong/broken', you should be able to verify X"
  - "After making a wrong technical claim — recovery pattern"
version: 1.0.0
---

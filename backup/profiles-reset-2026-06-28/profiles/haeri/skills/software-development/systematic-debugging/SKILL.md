...
The methodology above is language-agnostic. For specific debuggers:

| Language | Reference |
|---|---|
| **Python** (pdb, debugpy, breakpoint()) | `references/python-debugpy.md` |
| **Node.js** (`node inspect`, CDP, ndb) | `references/node-inspect-debugger.md` |
| **Hermes TUI** (Python + gateway + Ink UI layers) | `references/hermes-tui-commands.md` |
| **Python venv crash** (post-mortem example) | `references/python-venv-crash.md` |

### Quick decision tree for picking a debugger

- **Test fails, value is wrong** → use language-specific debugger on the failing test
- **Long-running process misbehaves** → use remote debugger (debugpy for Python, `--inspect-brk` for Node)
- **Hermes command missing in TUI** → read `references/hermes-tui-commands.md` (covers the 3-layer sync issue: Python registry → gateway → Ink UI)
- **Build/runtime crash on startup** → first reproduce, THEN apply Phase 1-4 methodology above

## Absorbed Skills (consolidated into umbrella, June 2026)

- `node-inspect-debugger` → `references/node-inspect-debugger.md`
- `python-debugpy` → `references/python-debugpy.md`
- `debugging-hermes-tui-commands` → `references/hermes-tui-commands.md`

## Related skills (kept standalone)
- `hermes-s6-container-supervision` (in `software-development/`) — About the s6-overlay tree in the Hermes Docker image, not a debugging methodology. Kept standalone.

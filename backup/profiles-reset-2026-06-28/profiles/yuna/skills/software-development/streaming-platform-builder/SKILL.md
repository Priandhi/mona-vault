...
## ICLIX Adult 21+ Scrapers (from `iclix-adult-21-plus-scrapers`)

Multi-source adult content scrapers: Eporner, HQporner, XNXX, HDZog. Anti-hotlink embed-iframe playback (NOT direct MP4). JSON-LD metadata extraction. Site triage: premium-paywalled vs free-aggregator. Age gate component: `templates/iclix-age-gate.jsx`. Site triage: `references/iclix-adult-site-triage.md`.

## ICLIX Anti-Ads Direct Play (from `iclix-anti-ads-direct-play`)

Custom HLS player bypassing iframe ads — scrapes m3u8 from VidLink/VidSrc via headless Chromium, plays through backend stream proxy. API details: `references/iclix-eporner-api.md`.

## ICLIX Anime Scrapers (from `iclix-anime-scrapers`)

Multi-source anime scrapers: AnimeUnity, Oploverz, Otakudesu, Samehadaku. Playwright via Python for JS-rendered sites. Jikan/AniList API fallback. Templates: `templates/iclix-anime-*`. References: `references/iclix-anime-*`. Verification: `scripts/iclix-verify-scraper.sh`.

## ICLIX Frontend Design (from `iclix-frontend-design`)

Dark/red/gold design system. Page structure, color tokens, component patterns, brand rules, deploy-build-restart cycle.
...
`web-streaming-platform` (now archived) was a narrower variant of this same domain. Its unique content has been merged:
- `references/embed-sources.md` — embeds catalog
- `references/iptv-m3u8-tested-2026-06.md` — already shared with this umbrella
- `references/iclix-vps-deployment.md` — ICLIX VPS deployment pattern
- `templates/server.js` — Express backend starter
- `scripts/discover-and-validate-iptv.sh` — IPTV/M3U8 source validator

When the user asks for a streaming platform, movie site, IPTV service, or ICLIX deployment, load THIS umbrella. The archived skill is no longer in `skills_list`.

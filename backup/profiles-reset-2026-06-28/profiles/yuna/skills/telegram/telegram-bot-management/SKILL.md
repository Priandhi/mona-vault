...
- **`telegram-trading-reports`** — Binance futures position snapshots formatted as plain-text Telegram reports. Added scripts: `close_losers.py`, `format_snapshot.py`, `emergency_sl_placement.py`.

When the user asks for trading reports, position snapshots, or PnL formatting destined for Telegram, load THIS umbrella.

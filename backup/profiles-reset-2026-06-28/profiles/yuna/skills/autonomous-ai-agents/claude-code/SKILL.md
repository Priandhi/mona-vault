...
OpenAI's autonomous coding agent. Install: `npm install -g @openai/codex`. Auth: `OPENAI_API_KEY` or Codex OAuth.

**Key differences from Claude Code:**
- Requires git repository (won't run outside one)
- Always use `pty=true` — interactive terminal app
...
Provider-agnostic, open-source AI coding agent. Install: `npm i -g opencode-ai@latest`.

**Key differences from Claude Code:**
- `opencode run "prompt"` for one-shot (no pty needed)
- Interactive TUI requires `pty=true`
- `/exit` is NOT valid — use Ctrl+C to exit
- `opencode pr 42` for built-in PR review
- Supports multiple providers (OpenRouter, Anthropic, etc.)
...
AWS's AI coding IDE/CLI. Has its own API keys (`ksk_` prefix) but NOT OpenAI-compatible.

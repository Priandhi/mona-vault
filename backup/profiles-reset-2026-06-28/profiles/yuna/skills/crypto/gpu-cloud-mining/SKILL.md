...
- **`vps-mining-setup`** — CPU mining (RandomX/Juno Cash) on VPS, swap mining, Telegram monitoring. Added references: `twitter-access-patterns.md`, `juno-pool-mining.md`.

This umbrella now covers both GPU cloud mining (Modal/RunPod/Vast) AND CPU/VPS mining (RandomX/Juno Cash). When user says "mine crypto", "setup mining", or asks about specific chains, load THIS skill.

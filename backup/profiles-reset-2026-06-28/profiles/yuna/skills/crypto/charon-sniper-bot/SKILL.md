...
- **Broken position (entryPrice=0):** If Jupiter quote fails AND `getTokenPrice()` returns null, the fallback calculates `price=0` and `tokenAmount=50B`. This creates a phantom position that blocks new buys ("Max positions reached"). Fix: (1) clean `data/positions.json` manually, (2) fix the fallback to use `sig.priceUsd` from Charon data as last resort, (3) pass `sig` object to `buyToken(mint, solAmount, config, sig)`.
...
- **`solana-sniper-bot`** — Earlier draft of the Solana sniper workflow. Unique references (`charon-api.md`, `jupiter-api.md`) merged into `references/`. Templates: `solana-keypair-convert.js`, `solana-bot-watcher.sh`, `solana-bot-autoswap.js` already in this umbrella's `templates/`.

This umbrella is now the single source of truth for Solana sniper bot work (Charon signals + Jupiter DEX + position management). Load THIS skill instead of searching for "sniper bot".

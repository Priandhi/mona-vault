...
- **`claude-code-mimo-setup`** — Setup for Claude Code CLI pointed at Xiaomi MiMo API (cheaper Anthropic alternative). The reference doc moved here: `references/mimo-claudecode-docs.md`. Quick setup commands (settings.json, regional endpoints, 1M context suffix) are inlined in that reference — load this umbrella when the user wants Claude Code with MiMo.

This umbrella is now the canonical LLM routing entry point: 9Router setup, OpenRouter free models, and Claude Code + MiMo provider combinations.

...
## Limit Order Tracking Pattern (Maker Fee Optimization)

**When to use:** For signal-driven entries, placing a LIMIT order at signal entry ± small offset (typically 0.3% below for LONG, 0.3% above for SHORT) saves maker fees vs market orders. But this means the order may NOT fill immediately — execution has two outcomes:

1. **Filled** — position opens, actual fill price available
2. **Not filled** — order sits in book, expires or gets cancelled

The executor must distinguish these and report to the notifier correctly. A naive "✅ EXECUTED" message after placing a limit order confuses the user (they think position is open when it isn't). **See `telegram-alert-integration.md` for notifier format.**

### Correct Executor Pattern

```python
def execute_trade(self, symbol, direction, entry, sl, tp1, tp2, tp3):
    result = {"success": False, "filled": False, "limit_price": 0,
              "symbol": symbol, "direction": direction,
              "entry": entry, "sl": sl, "tp1": tp1, "tp2": tp2, "tp3": tp3,
              "quantity": 0, "leverage": 0, "order_id": "", "error": ""}

    # ... set leverage, calculate position size ...

    # Place LIMIT order at offset
    offset_pct = 0.003  # 0.3% offset for maker fee
    if direction == "LONG":
        limit_price = entry * (1 - offset_pct)
    else:
        limit_price = entry * (1 + offset_pct)
    limit_price = self._round_price(symbol, limit_price)

    order = self.conn.limit_order(symbol, side, qty, limit_price, time_in_force="GTC")
    if order.get("_error"):
        result["error"] = f"Limit order failed: {order.get('msg', 'unknown')}"
        return result  # success=False, filled=False → notifier shows ❌ FAILED

    result["order_id"] = str(order.get("orderId", ""))

    # Poll for fill (up to 60s)
    filled = False
    for attempt in range(POSITION_POLL_MAX):
    ...
    if not filled:
        # Order in book, NOT filled — caller must NOT treat as open position
        result["success"] = True   # Order is "out there" — partial success
        result["filled"] = False
        result["limit_price"] = limit_price
        return result

    # Filled — get actual entry price and place SL/TP
    actual_entry = ...  # from positions data
...
### Caller Pattern (in auto.py / main loop)

```python
result = executor.execute_trade(...)

# Send execution report — notifier picks label from result['filled']
send_execution(
    symbol=result["symbol"], direction=result["direction"],
    entry=result["entry"], qty=result["quantity"],
...
if result["success"] and result.get("filled"):
    # Position is open — track in journal, set cooldown
    tracked[result["symbol"]] = {...}
    save_tracked(tracked)
elif result["success"] and not result.get("filled"):
    # Order in book — do NOT track as open position
    # Mark as "signalled" (already in scanner) to prevent duplicate limit orders
    # Wait for fill via background poll or soft-stop cron
    pass
```

### Common Bug: `max_lev` undefined in log statement

If the executor references a variable only defined in another method's scope, NameError fires only when the log line executes. Example from Dozero 2026-06-17:

```python
# calculate_position_size() defines max_lev locally
def calculate_position_size(self, symbol, entry, sl, leverage=None):
    if leverage is None:
        leverage = self.conn.get_max_leverage(symbol)
    margin = self._get_margin_per_trade()
    max_lev = leverage  # ← local var
    ...

# execute_trade() uses max_lev in a log statement, but it's NOT defined here
def execute_trade(self, ...):
    leverage = self._get_leverage_for_symbol(symbol, current_price)
    ...
    logger.info("[%s] Position: qty=%f, leverage=%d, margin=$%.2f",
                symbol, qty, max_lev, pos["margin"])  # ← NameError: max_lev
```

**Fix:** Use the local `leverage` variable (which IS defined in execute_trade scope), not the inner `max_lev` from calculate_position_size. This kind of cross-scope variable leak survives static review and only fires at runtime — unit tests for execute_trade should pass a real Binance connection to catch it.

...
These narrow skills were archived and their content merged into this umbrella:

- **`binance-futures-trading`** — Binance-specific USDT-M trading patterns (SL/TP algo orders, leverage brackets, dozero connection quirks). Unique references moved to `references/`: `sl-tp-order-limit.md`, `auto-sltp-manager.md`, `preflight-test-suite.md`, `dozero-connection-quirks.md`, `leverage-bracket-api.md`. Unique scripts in `scripts/`: `yuna_soft_stop.py`, `yuna_format.py`, `sl_tp_filter_test.py`.

- **`crypto-signal-scanner`** — Dual-mode signal scanner (momentum + SMC). Unique references: `professional-chart-mplfinance.md`, `smc-engine-integration.md`, `market-context-modules.md`, `cron-delivery-rules.md`, `smc-engine-notes.md`, `watchlist-price-alerts.md`. Note: `auto-sltp-manager.md` was shared with binance-futures-trading.

When the user asks about Binance futures, signal scanning, momentum/SMC reversal, or trading bot architecture, load THIS umbrella.

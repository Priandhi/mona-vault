...
These narrow skills were archived; their unique content lives in this umbrella's `references/`, `templates/`, `scripts/`:

- **`alpha-hunter-new-token-discovery`** — New token discovery workflow. Added: `api-examples.md`, `sniper-monitor-impl.md`, `browser-scraping-fallback.md`. (Plus the umbrella's existing `alpha-hunting-workflow.md`.)
- **`crypto-alert-bot`** — Telegram alert formatting patterns. Added: `macd-alert-format.md`, `clean-alert-implementation.py`, `chart-generation-mplfinance.md`, `telegram-topic-creation.md`, `command-center-architecture.md`, `alert-system-architecture.md`, `alpha-alert-template-design.md`, `mona-ai-group-setup.md`, `telegram-topic-api.md`, `hermes-secret-redaction-workaround.md`.
- **`crypto-data-scrapers`** — Free crypto data API catalog. Added: `cpu-mining-reference.md`.
- **`crypto-smart-money`** — EVM whale/smart-money tracker. Added: `dexscreener-project-links.md`, `multi-provider-fallback.md`, `dexscreener-api.md`, `clickable-html-links.md`, `clean-alert-format.md`, `goplus-security-api.md`, `deployer-detection-alchemy.md`, `zerion-api.md`.
- **`galxe-reverse-engineering`** — Galxe airdrop platform RE. Added: `galxe-gql-schema.md`.
- **`gandalf-level-8`** — Lakera Gandalf CTF breakthrough. Added: `lakera-gandalf-full-session.md`.
- **`web3-game-bypass-recon`** — Indie web3 game bypass methodology. Added: `owntown-case-study.md`.

When the user asks about any crypto/Web3 task — token discovery, smart-money tracking, alerts, airdrops, game bypass, CTF — load THIS umbrella.

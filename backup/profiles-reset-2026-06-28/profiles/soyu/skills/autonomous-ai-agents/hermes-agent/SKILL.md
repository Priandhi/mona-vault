...
## OpenClaw Superagent Framework

Self-improving, self-healing agent framework overlay (27 tools, 37 skills). Covers reflection engine, watchdog, vault, governor, HIDS, skill forge. Key integration patterns:
- Reflection → cron job every 6-24h (scan logs, learn patterns, auto-fix reversible issues)
- Watchdog → systemd/cron health check (monitor processes, auto-restart)
- Vault → SQLite address/macro storage (FROZEN — self-improve can't edit)
- FROZEN_PATHS: 18+ files self-improve CANNOT touch
- SAFE_AUTO_ACTIONS: only reversible, non-financial actions

Full framework: archived skill `openclaw-superagent-framework` (references/tool-import-fixes.md, references/integration-guide.md)

## Review Discipline

When reviewing technical proposals: flag REAL bugs, not invented ones. Only flag what you can verify. Wrong findings are FAR more expensive than missed issues. If wrong, own it fast.

Full patterns: `references/review-discipline-patterns.md`

## AI Image Generation

Generate images via HuggingFace FLUX (free), Gemini (free tier), or Pollinations.ai. Prefer HF FLUX — don't over-engineer.

Full guide: `references/ai-image-generation-guide.md`

## Self-Update & Codebase Merge

When user sends a zip artifact or says "perbarui"/"upgrade", execute immediately with default rules — don't ask. Preserve persona files (SOUL.md, USER.md, IDENTITY.md), update skills/scripts/references. For external codebase merges (OpenClaw, Superagent), only ADD new capabilities, never remove existing.

Full workflow: `references/agent-self-update-pattern.md`

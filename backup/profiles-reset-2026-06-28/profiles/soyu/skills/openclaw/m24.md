# m24 — MCP-Builder & Prompt Engineering (NEW in v4.1)

Dua kemampuan meta-dev: bangun **MCP server** (Python/TS) dengan best practices, dan **prompt engineering** (deteksi + fix masalah prompt otomatis). Komplemen m7 (AI builder) & m16 (software eng). Tool: `tools/mcp_builder.py`.
...
Python pakai `FastMCP` (`pip install mcp`); TS pakai `@modelcontextprotocol/sdk`. Untuk daftar/explore MCP yang ada → host punya registry (lihat juga m0 marketplace untuk skill, beda dari MCP server).
...
Loop optimasi prompt = **eval-driven** (pakai x5): ubah 1 elemen → eval → bandingkan pass_rate & variance → keep yang menang. Jangan ubah banyak sekaligus.

---

## Catatan
- MCP server = kode yang jalan → review sebelum expose; HTTP mode wajib auth (sama prinsip dashboard m17).
- Prompt engineering nyambung ke m7 (provider/model) & x5 (ukur perbaikan).
- Combo: m24 + m7 (LLM) + m16 (package/deploy server) + x5 (eval prompt).

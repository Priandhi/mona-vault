...
## Additional Resources (Absorbed from web-streaming-platform)

The legacy `web-streaming-platform` skill was consolidated into this umbrella on 2026-06-17. The files below were preserved under new names (suffix `-wsp`) to avoid collision with existing references in this umbrella:

- `references/embed-sources-wsp.md` — alternate embed source list (older test data, complementary to `embed-sources-tested.md`)
- `references/vps-deployment-wsp.md` — ICLIX VPS-specific deployment quirks (Tencent Cloud security group, port 80 reserved, tunnel-url-watcher, localhost.run fallback)
- `scripts/discover-and-validate-iptv-wsp.sh` — batch IPTV stream discovery + validation script
- `templates/server-wsp.js` — alternate Express backend reference template

Use these when you specifically need the older test results or the VPS-specific deployment notes that are not duplicated in the umbrella's primary references. For new projects, prefer the primary references (`embed-sources-tested.md`, `hls-proxy-implementation.md`).

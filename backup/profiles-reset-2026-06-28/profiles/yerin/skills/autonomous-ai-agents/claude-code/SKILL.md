...
12. **Start new sessions for distinct tasks** — sessions last 5 hours; fresh context is more efficient.
13. **Use `--no-session-persistence`** in CI to avoid accumulating saved sessions on disk.
14. **Code analysis pattern (Jun 2026):** For analyzing existing codebases, use SHORT focused queries instead of broad analysis. Example: "Apa masalah di token-filter.js?" works better than "Analisis semua kode di folder ini". Broad queries timeout; focused queries return detailed analysis. Strategy: (1) List files and their purposes, (2) Ask about specific files, (3) Ask about specific issues. This avoids timeouts and gets better results.
15. **Bug hunting workflow:** Claude Code excels at finding bugs when given specific files. It identified 5 critical issues in token-filter.js and screening.js in Jun 2026 session. Pattern: read file → ask Claude about specific concerns → apply fixes. For complex projects, break analysis into: (1) "List semua file JavaScript di folder ini", (2) "Analisis [specific-file.js]", (3) "Berdasarkan analisis tadi, berikan perbaikan dengan kode siap pakai".

## Alternative Coding Agents

### Kiro CLI (AWS)
Kiro is AWS's AI coding IDE/CLI — competitor to Claude Code. Has its own API keys (`ksk_` prefix) but **NOT OpenAI-compatible** — keys are for Kiro CLI headless mode only. See `references/kiro-api-investigation.md` for full API endpoint investigation results.

Install: `curl -fsSL https://cli.kiro.dev/install | bash`
Models: Claude Opus 4.8 (1M context), Sonnet 4.6, DeepSeek 3.2, etc.

## Verifying Auth Status
```bash
claude auth status --text  # Human-readable auth info
claude --version            # Version check
claude -p "echo hello" --output-format json --max-turns 1 --bare  # Quick connectivity test
```
The JSON output shows: model used, context window, cost, token counts, and session ID.
...
13. **Max turns budget** — For complex tasks (multi-file edits, config changes, debugging), 20-30 turns is often not enough. If Claude hits max_turns, the task is incomplete. Strategies: (a) break into smaller subtasks, (b) increase `--max-turns` to 50+, (c) use `--continue` to resume. On MiMo v2.5-pro, complex tasks like full-stack config changes consistently need 30+ turns.
14. **Heavy context consumption** — When Claude reads large files (config.yaml, .env, multiple source files), context fills quickly. On a 200K context window model, reading 5-10 large files can consume 50%+ of context. Keep tasks focused: "Fix X in file Y" works better than "Audit everything and fix all issues."

## Rules for Hermes Agents

0. **DELEGATE ON ERROR — don't debug yourself.** User explicitly said: "kalau kesulitan dan ada eror langsung suruh claude code kerja keras jangan mikir sendiri nanti lama proses nya." When you hit an error, wall of red text, or confusing failure — immediately delegate to Claude Code. Don't spend turns debugging manually. User values speed over self-learning. Also: don't ask if user wants Claude Code — just do it. User said "gas kok masih tanya."
...
11. **User prefers Claude Code for ALL coding** — not just heavy tasks. User said: "meskipun coding ringan pake aja kan gratis" (even light coding, use it since it's free via MiMo API). Always delegate coding to Claude Code regardless of complexity. Mona coordinates, Claude Code executes.

## Delivering Code Output to User

User 0xjosee hates code spam. Rules for presenting code/delegation results:

- **Don't dump raw code blocks indiscriminately.** If Claude Code produced files, summarize what changed and why — don't just paste the entire file contents.
- **Be direct.** Operator-to-operator style. No "Here's what I did:" preamble, no disclaimer before code.
- **Show only the relevant snippet** when the user asks about specific logic. If they ask "how does X work," show just the X part, not the whole file.
- **If output is large** (diff, full file, multi-file), summarize first — "Created auth.py: JWT implementation with 3 helper functions" — then offer to show specifics if needed.
- **No intro/welcome/remember-me** in responses. User is not new. Just answer.

## Cost Optimization: MiMo API Backend (Absorbed from claude-code-mimo-setup)

You can run Claude Code against Xiaomi MiMo API instead of Anthropic's API — same CLI, ~10x cheaper. Two endpoints:

| Method | BASE_URL | Key Format |
|--------|----------|------------|
| Pay-as-you-go | `https://api.xiaomimimo.com/anthropic` | `sk-xxxxx` |
| Token Plan (CN) | `https://token-plan-cn.xiaomimimo.com/anthropic` | `tp-xxxxx` |
| Token Plan (SG) | `https://token-plan-sgp.xiaomimimo.com/anthropic` | `tp-xxxxx` |

**⚠️ Regional endpoints matter!** A Singapore Token Plan key MUST use the Singapore BASE_URL, not the CN one — wrong region returns 401/empty.
...
See `references/mimo-claudecode-docs.md` for the full setup procedure, model routing, and rate-limit notes that were preserved from the legacy `claude-code-mimo-setup` skill (consolidated 2026-06-17).
